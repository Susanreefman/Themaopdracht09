...
```{r message=FALSE, warning=FALSE, fig.cap="Top: Histogram of age of first use of patients in groups; Bottom: Histogram of frequency of use by patients in groups" }
...
```{r}
data$disorder <- factor(ifelse(
  data$PSYPROB == 1, "Yes", "No"))

ggplot(data = data, aes(x=disorder, y=LOS))+
  geom_boxplot()
